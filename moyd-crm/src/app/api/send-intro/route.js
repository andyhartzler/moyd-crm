  ...
  try {
    ...
    for (const recipient of recipients) {
      try {
        ...
        let chatGuid = recipient.phone?.includes(';')
          ? recipient.phone
        ...
        let textResult
        let triedCreateChat = false

        // Handle iMessage failure
        if (!textResponse.ok || textResponse.status === 500) {
          const errorResult = await textResponse.json()
          console.log('❌ Text response error:', errorResult)

          // 🔥 If chat doesn't exist for iMessage, create iMessage chat WITH message
          if (!triedCreateChat && errorResult.error?.message?.includes('Chat does not exist') && chatGuid.startsWith('iMessage')) {
            console.log('⚠️ iMessage chat doesn\'t exist, creating iMessage chat WITH intro message...')
            triedCreateChat = true

            try {
              // 🔥 On macOS Big Sur+, message is REQUIRED when creating chat
              const createIMsgChatResponse = await fetch(
                `${BB_HOST}/api/v1/chat/new?password=${BB_PASSWORD}`,
                {
                  ...
                  body: JSON.stringify({
                    ...
                    service: 'iMessage',
                    message: introMessage  // 🔥 REQUIRED on Big Sur+
                  })
                }
              )

              if (createIMsgChatResponse.ok) {
                const chatResult = await createIMsgChatResponse.json()
                console.log('✅ iMessage chat created with message:', chatResult.data?.guid || 'success')

                // Chat created AND message sent! Set textResult
                textResult = chatResult

                // Update chatGuid for vCard sending later
                if (chatResult.data?.guid) {
                  chatGuid = chatResult.data.guid
                }
              } else {
                // Chat creation failed - log details
                const chatError = await createIMsgChatResponse.json()
                console.log('❌ iMessage chat creation failed:', {
                  status: createIMsgChatResponse.status,
                  error: chatError
                })
              }
            } catch (iMsgChatError) {
              console.log('⚠️ iMessage chat creation exception:', iMsgChatError.message)
            }
          }

          // If iMessage still failed, try SMS fallback
          if (!textResult && !triedSMS && chatGuid.startsWith('iMessage')) {
            console.log('⚠️ iMessage failed completely, retrying with SMS...')
            triedSMS = true

            // Create SMS chat WITH message (required on Big Sur+)
            console.log('📱 Creating SMS chat WITH intro message...')
            ...
            try {
              ...
              if (createChatResponse.ok) {
                ...
                console.log('✅ SMS chat created with message:', chatResult.data?.guid || 'success')
                ...
                const chatError = await createChatResponse.json()
                console.log('❌ SMS chat creation failed:', {
                  status: createChatResponse.status,
                  error: chatError
                })
                throw new Error(`Failed to create SMS chat: ${createChatResponse.status}`)
              }
            } catch (err) {
              console.log('❌ SMS chat creation exception:', err.message)
              throw new Error(`Failed to send SMS: ${err.message}`)
            }
          }

          // If we still don't have a result, fail
          if (!textResult) {
        ...
        // 🔥 Update member with intro_sent_at timestamp
        await supabase
          .from('members')
          .update({
