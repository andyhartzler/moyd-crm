    ...
    for (let i = 0; i < validRecipients.length; i++) {
      ...
      try {
        ...
        if (!response.ok || result.status !== 200) {
          // 🔥 If chat doesn't exist, create it and retry
          if (result.error?.message?.includes('Chat does not exist') || result.message?.includes('Chat does not exist')) {
            console.log(`⚠️ Chat doesn't exist for ${recipient.name}, creating chat...`)

            try {
              // Determine service type (iMessage or SMS)
              const service = chatGuid.startsWith('iMessage') ? 'iMessage' : 'SMS'
              ...
              if (createChatResponse.ok) {
                ...
                console.log('✅ Chat created:', chatResult.data?.guid || 'success')

                // Use the actual chat GUID from response if available
                let actualChatGuid = chatGuid
                ...
                // Retry sending with the actual chat GUID
                ...
                if (!retryResponse.ok || retryResult.status !== 200) {
                  console.error(`❌ Retry failed for ${recipient.name}:`, retryResult)
                  progress.failed.push({
                    name: recipient.name,
                    phone: recipient.phone,
                    error: retryResult.error?.message || retryResult.message || 'Failed after creating chat'
                  })
                } else {
                  console.log(`✅ Sent to ${recipient.name} after creating chat`)
                  progress.sent++
                  threads.push({
              ...
              console.error(`❌ Failed to create chat for ${recipient.name}:`, chatError)
              progress.failed.push({
                name: recipient.name,
                phone: recipient.phone,
            ...
            // Different error - not chat-related
